...
public class AlbumOverviewActivity extends Activity implements LoaderCallbacks<Cursor> {
	...
	protected void onResume() {
		...
		cursorAdapter = new SimpleCursorAdapter(this, R.layout.album_overview_item, null, 
				new String[] { 
					Client.Album.NAME,
		...
		cursorAdapter.setViewBinder(new ViewBinder() {
			...
			public boolean setViewValue(View view, Cursor cursor, int columnIndex) {
				...
				final String thumbnailUriString = cursor.getString(2);
				...
				AsyncTask<Void, Void, Void> asyncTask = new AsyncTask<Void, Void, Void>() {
					...
					protected Void doInBackground(Void... params) {
						try {
							...
							try {
								int imageLength = getResources().getDimensionPixelSize(R.dimen.image_width);
		...
		/*
		 * gridview.setOnItemClickListener(new OnItemClickListener() { public void onItemClick(AdapterView<?> parent, View
		 * v, int position, long id) { Toast.makeText(AlbumOverviewActivity.this, "" + position, Toast.LENGTH_SHORT).show();
		 * } });
		 */
